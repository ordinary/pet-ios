...
#import "PetMenuBarView.h"
#import "PetMenuBar.h"
#import "PetMenuBarDelegate.h"

#import <QuartzCore/QuartzCore.h>

@implementation PetMenuBarView
@synthesize menuBar = _menuBar;
@synthesize selectedItemIndex = _selectedItemIndex;
@synthesize delegate = _delegate;

- (void)setDelegate:(id <PetMenuBarDelegate>)theDelegate {
...
- (void)fillParams {
    ...
    PetMenuBar *menuBar = self.menuBar;
    ...
    buttonHighPetColor = [self colorFromCode:0x4d9c5d];
    titleHighPetColor = [self colorFromCode:0xe9f4e9];
    ...
    if ([(id)self.delegate respondsToSelector:@selector(colorOfButtonHighPetInMenuBar:)])
    {
        buttonHighPetColor = [self.delegate colorOfButtonHighPetInMenuBar:menuBar];
    }
    
    if ([(id)self.delegate respondsToSelector:@selector(colorOfTitleHighPetInMenuBar:)])
    {
        titleHighPetColor = [self.delegate colorOfTitleHighPetInMenuBar:menuBar];
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    /**< Highlight Background */
    {
        if (self.selectedItemIndex != NSNotFound)
        {
            ...
            float highPetVpadding = buttonRad/2.0f;
            float highPetHpadding = buttonRad/2.0f;
            CGRect highPetRect = CGRectMake(currentX + highPetHpadding, CGRectGetMinY(bkgrdRect) + highPetVpadding, 
                                              itemWidth - 2*highPetHpadding, bkgrdRect.size.height - 2*highPetHpadding);            
            [self addRoundCornerRect:highPetRect 
                    withCornerRadius:buttonRad
                           inContext:context];
            CGContextSetFillColorWithColor(context, buttonHighPetColor.CGColor);
            CGContextFillPath(context);
        }
    }
    ...
    /**< Titles */
    {
        ...
        for (int i = 0; i < itemCount; i++)
        {
            ...
            {
                CGContextSetStrokeColorWithColor(context, titleHighPetColor.CGColor);
                CGContextSetFillColorWithColor(context, titleHighPetColor.CGColor);
            }
...
- (void)tapGesRecoDetected:(UITapGestureRecognizer *)gesReco {
    ...
    if (activeIndex != NSNotFound)
    {
        ...
        if (self.menuBar.barStyle == PetMenuBarStyleButton)
        {
            self.selectedItemIndex = activeIndex;
            [self blink];
        }
        else if (self.menuBar.barStyle == PetMenuBarStyleItem)
        {
            self.selectedItemIndex = activeIndex;
